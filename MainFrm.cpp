...
struct Candidate {	// 存放候选词信息的结构
  long/*short*/  offset, length; // 候选词在输入串中的起点，长度
  long/*short*/  goodPrev;       // 最佳前趋词的序号
  double/*float*/ fee, sumFee;   // 候选词的费用，路径上的累计费用
} Candidates[100];  // 假定最多100个候选词

long/*short*/ getTmpWords(CString& s)
{
...
CString  SegmentHzStrMP (CString  s1)
{
  ...
  long/*short*/ n=getTmpWords(s1); // 取出s1中的全部候选词：
  long/*short*/ minID=-1;      //设最小费用路径的终点词的序号为-1
  long/*short*/ i;
